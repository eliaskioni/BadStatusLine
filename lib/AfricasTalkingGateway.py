...
class AfricasTalkingGateway:
    def __init__(self, username_, apiKey_, environment_='production'):
        ...
        # This commented sendMessage is clean as provided by Africa's Talking.
        # Please uncomment to check the case of BadStatus Exception
        # This exception is not caught while sending a single message.
        # But with many numbers - I have personally tested 400 it starts to misbehave.
    ...
    def sendMessage(self, to_, message_, from_=None, bulkSMSMode_=1, enqueue_=0, keyword_=None, linkId_=None,
        ...
        try:
            ...
            logger.exception("Exception zone")
            for retries in range(20):
                try:
                    logger.info("Step One Handling Initial Excemption")
                    response = self.sendRequest(self.getSmsUrl(), parameters)
                    logger.info("Step One Passed Well")
                    break
                except Exception:
                    logger.info("Step Two")
                    logger.exception("Exception zone Two")
                    try:
                        logger.info("Amber")
                        response = self.sendRequest(self.getSmsUrl(), parameters)
                        logger.info("Passed Well -  will break out of loop soon")
                        break
                    except Exception:
                        logger.exception("Exception zone Two - Wait before making the call again. Dirty fix")
                        logger.info("Returning control to loop - Maximum loop iterations is 20")
                        logger.info("currently retrying at {}".format(str(retries)))
                        sleep(5)
